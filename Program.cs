...
#if DEBUG
          .UseUrls("http://localhost:5100;https://localhost:5101;http://*:5100;https://*:5101")//指定Kestrel将侦听的URL。
#else
...
namespace WebEdu_LocalVersion_YuQin_DotNetCore21
{
    public class Program
    {
        public static void Main(String[] args)
        {
            ...
            **/
            WebApplicationBuilder webApplicationBuilder = WebApplication.CreateBuilder(args);
            //webApplicationBuilder.WebHost.UseUrls("http://localhost:5100;https://localhost:5101;http://*:5100;https://*:5101");//指定Kestrel将侦听的URL。
            webApplicationBuilder.WebHost.UseUrls("http://localhost:5100;https://localhost:5101;http://*:5100;https://*:5101");//指定Kestrel将侦听的URL。可以设置在appsettings.json中，使用JIT编译的方式获取（在此选用）。也可以在代码中硬编码设置。也可以在命令行中指定参数。
